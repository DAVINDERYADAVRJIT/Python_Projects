...
bnames_year=bnames[bnames['year']>=2011]
bnames_year.set_index('year',inplace=True)
#bnames_top5['sex'].value_counts()
bnames_top5_agg=bnames_year.groupby(['sex','name'],as_index=False)['births'].sum()
bnames_top5=bnames_top5_agg.sort_values(['sex','births'],ascending=[True,False]).groupby(['sex']).head()
...
bnames2.head()
tot_birth=bnames2.groupby(['year'])['births'].transform('sum')
bnames2['prop_births']=bnames['births']/tot_birth
bnames2.sort_values(['prop_births'],ascending=False).head()
...
names=pd.DataFrame()
grouped_names = bnames.groupby(['name','sex'], as_index=False)['births'].sum()
more_than_1000 = grouped_names.loc[grouped_names['births']>1000]
cols = ['name','sex','total']
more_than_1000.columns=cols
max_vals= bnames.groupby(['name','sex'], as_index=False)['births'].max()
cols1=['name','sex','max']
max_vals.columns=cols1
names=pd.merge(more_than_1000,max_vals,on=['name','sex'])
names['trendiness']=names['max']/names['total']
top10_trendy_names=names.sort_values('trendiness', ascending = False).head(10)
...
def get_data(name, sex):
    name_sex = ((bnames['name'] == name) & 
                (bnames['sex'] == sex))
    data = bnames[name_sex].merge(lifetable_2016_s,left_on=['year','sex'],
                                  right_on=['year','sex'])
...
# -- YOUR CODE HERE --
bnames.sort_values(['births'],ascending=False).head(10)
